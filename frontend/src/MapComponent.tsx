...
const MapComponent: React.FC<Props> = ({ 
  ...
  // Popup Size based on Zoom
  const isFarView = zoom < 14;
  const popupWidth = isFarView ? 320 : 550;
  const popupHeight = isFarView ? 240 : 412;
    ...
    <div className="relative w-full h-full bg-slate-100">
      ...
      >
        ...
        <Source id="track-source" type="geojson" data={trackGeoJSON}>
          <Layer id="track-layer" type="line" paint={{ 'line-color': mapStyle === 'satellite' ? '#ffffff' : '#3b82f6', 'line-width': 6, 'line-opacity': rangeSelection ? 0.2 : 0.8 }} />
        </Source>

        {highlightGeoJSON && (
          <Source id="highlight-source" type="geojson" data={highlightGeoJSON}>
            <Layer id="highlight-layer" type="line" layout={{ 'line-cap': 'round', 'line-join': 'round' }} paint={{ 'line-color': '#f43f5e', 'line-width': 10, 'line-opacity': 1, 'line-blur': 0.5 }} />
          </Source>
        )}
        ...
        {(data.landmarks || []).map((lm, i) => (
          <Marker key={`lm-${i}`} latitude={lm.lat} longitude={lm.lon} anchor="bottom">
            <div className="flex flex-col items-center group">
              ...
              <div className="p-2 bg-indigo-600 rounded-full border-2 border-white shadow-2xl scale-110"><MapPin size={20} className="text-white fill-white/20" /></div>
            </div>
          </Marker>
        ))}
          ...
          <React.Fragment key={`dm-${i}`}>
            <Marker latitude={dm.start.lat} longitude={dm.start.lon} anchor="bottom">
              <div className="flex flex-col items-center scale-125">
                <div className="bg-emerald-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full mb-1 shadow-lg border border-white uppercase tracking-tighter">DAY {dm.day} START</div>
            ...
            <Marker latitude={dm.end.lat} longitude={dm.end.lon} anchor="bottom">
              <div className="flex flex-col items-center scale-125">
                <div className="bg-rose-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full mb-1 shadow-lg border border-white uppercase tracking-tighter">DAY {dm.day} END</div>
        ...
        <div className="absolute bottom-4 right-4 z-10">
          <button onClick={e => { e.stopPropagation(); setMapStyle(prev => prev === 'outdoors' ? 'satellite' : 'outdoors'); }}
            ...
            <Layers size={18} /> {mapStyle === 'outdoors' ? '衛星圖' : '地圖'}
          </button>
        </div>
        ...
        {visibleGroups.map((group, idx) => (
          <Marker key={`photo-group-${idx}`} latitude={group[0].lat} longitude={group[0].lon} anchor="center" onClick={e => { e.originalEvent.stopPropagation(); setActivePhotoGroup(group); }}>
            <div className={`p-1.5 bg-white rounded-full shadow-2xl cursor-pointer hover:bg-yellow-400 transition-all border border-slate-100 ${activePhotoGroup === group ? 'ring-4 ring-yellow-500/50 scale-150' : 'scale-110'}`}>
              ...
              {group.length > 1 && zoom > 14 && <span className="absolute -top-2 -right-2 bg-rose-500 text-white text-[8px] px-1.5 py-0.5 rounded-full font-black shadow-lg border border-white">{group.length}</span>}
            </div>
          </Marker>
        ))}
            ...
            <div className="flex flex-col" style={{ width: `${popupWidth}px` }} onClick={e => e.stopPropagation()}>
              <div className="relative group overflow-hidden rounded-t-lg bg-slate-950 flex items-center justify-center" style={{ width: `${popupWidth}px`, height: `${popupHeight}px` }}>
                ...
                {activePhotoGroup.length > 1 && (
                  <><button onClick={(e) => { e.stopPropagation(); setCurrentPhotoIdx(prev => (prev > 0 ? prev - 1 : activePhotoGroup.length - 1)); }} className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-black/40 hover:bg-black/70 text-white rounded-full z-10 transition-colors"><ChevronLeft size={isFarView ? 20 : 32} /></button>
                    <button onClick={(e) => { e.stopPropagation(); setCurrentPhotoIdx(prev => (prev < activePhotoGroup.length - 1 ? prev + 1 : 0)); }} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-black/40 hover:bg-black/70 text-white rounded-full z-10 transition-colors"><ChevronRight size={isFarView ? 20 : 32} /></button></>
                )}
              </div>
              <div className="p-4 bg-white border-t border-slate-100 min-h-[80px]">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex flex-col">
                    ...
                    <h3 className="font-bold text-slate-800 text-sm truncate" style={{ maxWidth: `${popupWidth - 150}px` }}>{activePhoto.filename}</h3>
                  </div>
                ...
                <div className="flex justify-between items-center text-xs text-slate-400">
                   ...
                   {!isFarView && <p>海拔 {Math.round(activePhoto.ele)}m {activePhotoGroup.length > 1 ? `| ${currentPhotoIdx + 1} / ${activePhotoGroup.length}` : ''}</p>}
                </div>
              </div>
            </div>
          </Popup>
