...
const MapComponent: React.FC<Props> = ({ 
  ...
  const popupWidth = isMobile ? Math.min(windowWidth - 40, 380) : (zoom < 14 ? 350 : 550);
  const popupHeight = popupWidth * 0.75;

  useEffect(() => {
    if (mapCenterPoint && mapRef.current) {
      mapRef.current.easeTo({ center: [mapCenterPoint.lon, mapCenterPoint.lat], duration: 800 });
    }
  }, [mapCenterPoint]);
  ...
  useEffect(() => {
    if (activePhotoGroup) {
      ...
      if (mapRef.current) {
        mapRef.current.easeTo({ center: [activePhotoGroup[0].lon, activePhotoGroup[0].lat], duration: 800 });
      }
    }
  }, [activePhotoGroup]);
  ...
  const trackGeoJSON: any = useMemo(() => ({
    ...
    geometry: {
      ...
      coordinates: (data.trackSegments || []).map(seg => seg.map(p => [p.lon, p.lat, p.ele]))
    }
  }), [data.trackSegments]);

  const highlightGeoJSON: any = useMemo(() => {
    if (!rangeSelection || rangeSelection.length < 2) return null;
    return { type: 'Feature', geometry: { type: 'LineString', coordinates: rangeSelection.map(p => [p.lon, p.lat, p.ele]) } };
  }, [rangeSelection]);
    ...
    <div className="relative w-full h-full bg-slate-100">
      ...
      >
        ...
        {activePhotoGroup && activePhoto && (
          <Popup anchor="bottom" longitude={activePhotoGroup[0].lon} latitude={activePhotoGroup[0].lat} onClose={() => setActivePhotoGroup(null)} closeButton={true} closeOnClick={false} maxWidth={`${popupWidth + 20}px`} className="z-50 photo-popup">
